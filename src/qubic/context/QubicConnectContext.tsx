...
if (typeof window !== 'undefined') {
  try {
    ...
    if (qubicLib?.Crypto) Crypto = qubicLib.Crypto;
    if (qubicLib?.SIGNATURE_LENGTH) SIGNATURE_LENGTH = qubicLib.SIGNATURE_LENGTH;
    ...
    if (!Crypto) {
      try {
        // @ts-ignore
        const cryptoModule = require('@qubic-lib/qubic-ts-library/dist/crypto');
        Crypto = cryptoModule.default || cryptoModule;
        if (cryptoModule.SIGNATURE_LENGTH) SIGNATURE_LENGTH = cryptoModule.SIGNATURE_LENGTH;
      } catch (e) {}
    }
    if (!QubicTransaction) {
      try {
...
export function QubicConnectProvider({ children }: QubicConnectProviderProps) {
  ...
  const qHelper = QubicHelper ? new QubicHelper() : null;
  ...
  const getSignedTx = async (tx: Uint8Array | any): Promise<{ tx: Uint8Array }> => {
    if (!wallet || !connectTypes.includes(wallet.connectType)) {
      throw new Error(`Unsupported connectType: ${wallet?.connectType}`);
    }

    if (!QubicHelper || !Crypto) {
      throw new Error("Qubic libraries not installed. Please install @qubic-lib/qubic-ts-library");
    }

    const isQubicTransaction = QubicTransaction && tx instanceof QubicTransaction;
    const processedTx = isQubicTransaction ? await tx.build("0".repeat(55)) : tx;
  ...
  const vaultFileConnect = async (selectedFile: File, password: string): Promise<any> => {
